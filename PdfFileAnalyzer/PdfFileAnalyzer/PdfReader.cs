...
//		Upgrade to VS 2022 and .NET 6.0
//
/////////////////////////////////////////////////////////////////////
...
namespace PdfFileAnalyzer
	...
	public class PdfReader : IDisposable
		...
		/// </summary>
		public static readonly string VersionNumber = "3.0.0";

		/// <summary>
		/// Library revision date
		/// </summary>
		public static readonly string VersionDate = "2022/03/05";
		...
		public bool OpenPdfFile
			...
			SafeFileName = FileName[(FileName.LastIndexOf('\\') + 1)..];
		...
		internal bool TestEncryptionSupport()
			...
			if (R == 4 && V == 4)
				...
				if (!StdCFDict.FindValue("/Length").GetInteger(out int Len) || Len != 16) return false;
		...
		internal void SetReaderActive()
			...
			foreach (PdfIndirectObject Obj in ObjectArray) if (Obj != null && Obj.FilePosition != 0) Obj.ReadObject();

			// get length of all streams
			// it must be done after all indirect objects have been read
			foreach (PdfIndirectObject Obj in ObjectArray) if (Obj != null && Obj.ObjectType == ObjectType.Stream) Obj.GetStreamLength();
			...
			if (DecryptionStatus != DecryptionStatus.FileNotProtected)
				{
				DecryptCtrl Ctrl = new(Encryption, 0, EncryptionDict);
			...
			foreach (PdfKeyValue KeyValue in Catalog.Dictionary.KeyValueArray)
				...
				if (RefNo == 0) continue;
				ObjectArray[RefNo].PdfObjectType = KeyValue.Key;
		...
		public PdfOp[] ParseContents
			...
			PdfByteArrayParser PC = new(this, Contents, true);
			PC.ReadFirstChar();

			List<PdfOp> OpArray = new();
			List<PdfBase> ArgStack = new();
		...
		private void ValidateFile()
			...
			if (ParseFile.ParseNextItem().ToKeyWord == KeyWord.XRef)
				...
				for (; ; )
					...
					if (!ParseFile.ParseNextItem().GetInteger(out int ObjectCount)) throw new ApplicationException("Cross reference Table error");
		...
		internal int ReadXRefStream
			...
			PdfIndirectObject XRefObj = new(this, XRefObjNo, XRefPos);
