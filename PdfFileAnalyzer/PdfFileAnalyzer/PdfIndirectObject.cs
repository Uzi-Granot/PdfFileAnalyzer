...
namespace PdfFileAnalyzer
	...
	public class PdfIndirectObject : PdfObject
		...
		public byte[] ReadStream()
			{
			// stream is empty
			if(StreamLength == 0) return Array.Empty<byte>();
		...
		public void BuildContentsArray()
			...
			if(ContentsValue.IsEmpty)
				{
				ContentsArray = Array.Empty<PdfIndirectObject>();
				return;
				}
		...
		public byte[] PageContents()
			...
			if(ContentsArray.Length == 0) return Array.Empty<byte>();
		...
		internal void ReadObject()
			...
			if(Value.IsDictionary)
				...
				// set special object
				if(ObjectTypeStr != null) PdfObjectType = ObjectTypeStr;
		...
		internal void ProcessObjectsStream()
			...
			PdfByteArrayParser PC = new(Reader, ByteArray, false);
			PC.ReadFirstChar();
			for(int Index = 0; Index < ObjectCount; Index++)
				...
				if(ReadObject == null) continue; // throw new ApplicationException("Cross reference object stream: object not found");
			...
			for(int Index = 0; Index < ObjectCount; Index++)
				...
				if(Obj.IsDictionary)
					...
					// set special object
					if(ObjectTypeStr != null) Child.PdfObjectType = ObjectTypeStr;
					}
		...
		/// <returns>Output buffer</returns>
		internal static byte[] FlateDecode
				(
				byte[] InputBuffer
				)
			{
			...
			if((Header & 0xf00) == 0x800)
				{
				// create input stream
				MemoryStream InputStream = new(InputBuffer, 2, InputBuffer.Length - 6);

				// create output memory stream to receive the decompressed buffer
				MemoryStream OutputStream = new();

				// deflate decompression object
				DeflateStream Deflate = new(InputStream, CompressionMode.Decompress, true);
		...
		/// <param name="InputBuffer">Input buffer</param>
		/// <returns>Output buffer</returns>
		internal static byte[] LZWDecode
				(
				byte[] InputBuffer
				)
			{
		...
		/// <param name="InputBuffer">Input buffer</param>
		/// <returns>Output buffer</returns>
		internal static byte[] Ascii85Decode
				(
				byte[] InputBuffer
				)
			{
