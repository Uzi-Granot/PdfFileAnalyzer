...
public class CryptoEngine : IDisposable
    ...
    internal byte[] DecryptByteArray
		...
		if(EncryptionType == EncryptionType.Aes128)
			...
			CipherStream = new MemoryStream(CipherText, 16, CipherText.Length - 16);
			CryptoStream = new CryptoStream(CipherStream, Decryptor, CryptoStreamMode.Read);
