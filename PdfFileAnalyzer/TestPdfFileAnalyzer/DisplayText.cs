...
namespace TestPdfFileAnalyzer
	...
	internal partial class DisplayText : Form
		...
		private Rectangle DisplayRect = new();
		private Bitmap Image;
		private Rectangle ImageRect = new();
		...
		private void OnViewContents(object sender, EventArgs e)
			...
			DisplayText Dialog = new(DisplayMode.Contents, Reader, ReaderObject, TempByteArray);
			Dialog.ShowDialog();
			return;
			}
		...
		private bool LoadStream()
			...
			if(ReaderObject.PdfObjectType == "/Page")
				...
				if(TempByteArray == null)
					MessageBox.Show("Stream is compressed by unsuported filter");
		...
		private void OnSaveToFile(object sender, EventArgs e)
			...
			switch(Mode)
				...
				case DisplayMode.PdfSummary:
					...
					SFD.FileName = string.Concat(SafeFileName.AsSpan(0, SafeFileName.Length - 4), "Analysis.txt");
					if(SFD.ShowDialog() != DialogResult.OK)
						return;
					using(StreamWriter SaveFile = new(SFD.FileName))
				...
				case DisplayMode.Stream:
					...
					using(FileStream OutStream = new(SFD.FileName, FileMode.Create, FileAccess.Write, FileShare.None))
						{
						using BinaryWriter OutFile = new(OutStream);
